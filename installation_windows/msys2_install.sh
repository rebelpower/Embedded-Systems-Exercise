...
avrtools=$download_dir/avr8-gnu-toolchain
...
if grep "^source ~/downloads/activate_msys2.sh" ~/.bashrc
then
    echo
    echo "The line source ~/downloads/activate_msys2.sh is already added to your /.bashrc"
    echo "Not adding it twice!"
else
    while true; do
        echo
        echo "Do you want to add 'source ~/downloads/activate_msys2.sh' to the end of your"
        ...
        read -p "(Y/N)? " res
        case $res in
            [Yy]* ) echo "source ~/downloads/activate_msys2.sh" >> ~/.bashrc; break;;
            [Nn]* ) break;;
            * ) echo "Invalid answer";;
        esac
    done
